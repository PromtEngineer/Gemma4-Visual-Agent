"""
Vision Agent Studio — Agentic Loop with Re-planning (NVIDIA CUDA / PyTorch)
==========================================================================
Same pipeline as the repo-root MLX build: Falcon Perception + Gemma 4 E4B-it.
This variant targets Linux + NVIDIA GPU (e.g. DGX Spark GB10), using
falcon-perception[torch] and Hugging Face Transformers for Gemma 4.

Run from this directory: ``python vision_studio.py``
"""
...
falcon_model = falcon_tokenizer = falcon_args = falcon_device = None
gemma_model = gemma_processor = None
FALCON_ID = os.environ.get("FALCON_HF_MODEL_ID", "tiiuae/Falcon-Perception")
GEMMA_ID = os.environ.get("GEMMA_HF_MODEL_ID", "google/gemma-4-E4B-it")
...
STEP_DIR = os.path.join(_PKG_DIR, "step_outputs")
...
def _load_falcon():
    global falcon_model, falcon_tokenizer, falcon_args, falcon_device
    if falcon_model is not None:
        return
    from falcon_perception import load_and_prepare_model, setup_torch_config, PERCEPTION_MODEL_ID
    setup_torch_config()
    hf_local = os.environ.get("FALCON_HF_LOCAL_DIR") or None
    if hf_local == "":
        hf_local = None
    compile_model = os.environ.get("FALCON_TORCH_COMPILE", "0").lower() not in ("0", "false", "no")
    dtype = os.environ.get("FALCON_TORCH_DTYPE", "bfloat16")
    if dtype not in ("bfloat16", "float32", "float"):
        dtype = "bfloat16"
    device = os.environ.get("CUDA_DEVICE") or None
    if device == "":
        device = None
    falcon_model, falcon_tokenizer, falcon_args = load_and_prepare_model(
        hf_model_id=FALCON_ID or PERCEPTION_MODEL_ID,
        hf_revision=os.environ.get("FALCON_HF_REVISION", "main"),
        hf_local_dir=hf_local,
        device=device,
        dtype=dtype,
        compile=compile_model,
    )
    falcon_device = falcon_model.device
...
def _load_gemma():
    ...
    try:
        ...
        raise ImportError(
            "Loading Gemma 4 requires a transformers build that defines "
            "AutoModelForMultimodalLM and Gemma4 (not present in many PyPI 4.5x wheels). "
            "Install the current development wheel, then restart:\n"
...
def _detect(img, query, task="segmentation"):
    ...
    max_len = getattr(falcon_args, "max_seq_len", None) or 4096
    patch = getattr(falcon_args, "spatial_patch_size", 16)
    batch = process_batch_and_generate(
        falcon_tokenizer,
        [(img.convert("RGB"), prompt)],
        max_length=max_len,
        min_dimension=256,
        max_dimension=1024,
        patch_size=patch,
    )
    batch = {k: (v.to(falcon_device) if torch.is_tensor(v) else v) for k, v in batch.items()}
    eos = falcon_tokenizer.eos_token_id
    end_q = getattr(falcon_tokenizer, "end_of_query_token_id", None)
    stop_token_ids = [eos] if end_q is None else [eos, end_q]
    ...
    masks_rle = getattr(aux[0], "masks_rle", None) or []
    dets, i, mi = [], 0, 0
    while i < len(bboxes):
        d = {}
        if "x" in bboxes[i]:
...
def _vlm(img, prompt):
    ...
    gen_kw = {"max_new_tokens": 512}
    if os.environ.get("GEMMA_DO_SAMPLE", "1").lower() not in ("0", "false", "no"):
        gen_kw["temperature"] = 0.1
        gen_kw["do_sample"] = True
    else:
        gen_kw["do_sample"] = False
    with torch.inference_mode():
        outputs = gemma_model.generate(**inputs, **gen_kw)
    text = gemma_processor.decode(outputs[0][input_len:], skip_special_tokens=True)
    return text.strip()


def predictions_from_dets(dets):
    """Map internal _detect dicts to the prediction schema used by ``app.py``."""
    out = []
    for d in dets:
        pred = {}
        if "bbox" in d:
...
def run_falcon_perception(image: Image.Image, query: str, task: str = "segmentation"):
    """Falcon detection → structured predictions (for Gradio image apps)."""
    _ensure()
    return predictions_from_dets(_detect(image, query, task=task))


def run_gemma_reasoning(image: Image.Image, prompt: str) -> str:
    _ensure()
    return _vlm(image, prompt)
    ...
    with demo:
        ...
        <div style="text-align:center; padding:20px 0 8px;">
            ...
            <p style="font-size:14px; color:#9ca3af; margin:4px 0 0;">
                Falcon Perception + Gemma 4 &mdash; agentic step-by-step visual reasoning (PyTorch / CUDA)
            </p>
        </div>
        """)
